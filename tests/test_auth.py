...
from oura_mcp.auth import AuthError, OuraOAuth, Tokens, TokenStore
from oura_mcp.config import TOKEN_URL, Settings
...
async def test_no_tokens_points_at_auth_command(tmp_path):
    cfg = settings(tmp_path)
    with pytest.raises(AuthError, match="oura-mcp auth"):

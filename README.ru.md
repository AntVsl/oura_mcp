...
`https://твой-домен/mcp`. Ни client_id, ни секрет вводить не надо — Claude
зарегистрируется сам, а страница согласия спросит тот же `OURA_MCP_TOKEN`.

Работает это, если задан `OURA_PUBLIC_URL`: он включает встроенный сервер
авторизации OAuth 2.1 с динамической регистрацией клиентов. Статические
заголовки claude.ai тоже понимает, но поле для них в beta с ограниченной
раскаткой — в диалоге его может не быть вовсе.

Коннектор заводится один раз в вебе и дальше доступен во всех клиентах Claude
одного аккаунта, включая приложение на айфоне. Подробности и отладка —
